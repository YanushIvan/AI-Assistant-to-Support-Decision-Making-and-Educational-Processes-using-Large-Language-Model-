...
Parameter-efektívne doučenie (PEFT, z angl.\ \emph{Parameter-Efficient Fine-Tuning}) predstavuje skupinu metód, ktoré mrazené pôvodné váhy modelu a trénujú len malý počet nových parametrov. Výsledkom je výrazné zníženie pamäťových a výpočtových nárokov pri zachovaní podstatnej časti adaptačného efektu.
...
\begin{description}
    ...
    \item[PEFT] Knižnica od Hugging Face implementujúca metódy parameter-efektívneho doučenia. Použité komponenty: \texttt{LoraConfig}, \texttt{get\_peft\_model} a \texttt{prepare\_model\_for\_kbit\_training}. Verzia: 0.18.x.
