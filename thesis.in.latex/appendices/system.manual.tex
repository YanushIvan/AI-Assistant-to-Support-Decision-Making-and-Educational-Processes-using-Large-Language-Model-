...
Táto príručka opisuje internú štruktúru systému a hlavné funkcie. Text je určený pre údržbu a ďalší vývoj. Cesty sú uvádzané relatívne k rootu repozitára.
...
\begin{description}
	\item[\texttt{lifespan()}] Načítava model pri štarte a uvoľňuje zdroje pri ukončení (idempotentne cez \texttt{ModelService}).
...
Modul mountuje statické súbory na \texttt{/static} (ak adresár existuje) a registruje \texttt{chat\_router} s prefixom \texttt{/api}. Spustenie \texttt{uvicorn.run()} používa \texttt{Settings.HOST}, \texttt{PORT}, \texttt{RELOAD} a \texttt{LOG\_LEVEL}.

\textbf{\path{energy_chat/api/routes/chat.py}} obsahuje API routy.
\begin{description}
	\item[\texttt{get\_models()}] Vracia zoznam modelov z konfigurácie.
	\item[\texttt{answer\_prompt()}] Prepína model podľa \texttt{request.model}, skladá správy (so system promptom len pre adaptéry) a volá generovanie.
...
\begin{description}
	...
	\item[\texttt{Settings.MODELS}] Skladá zoznam modelov a cesty k adapterom (relatívne k rootu repozitára).
	\item[\texttt{Settings.LORA\_ADAPTER\_PATH}] Vráti predvolenú cestu k LoRA adapteru (legacy hodnota; runtime používa \texttt{MODELS}).
	\item[\texttt{Settings.SYSTEM\_PROMPT}] Systémový prompt pre modely s adaptérom.
	\item[\texttt{Settings.MAX\_NEW\_TOKENS}, \texttt{TEMPERATURE}, \texttt{TOP\_K}, \texttt{TOP\_P}] Generačné parametre.
	\item[\texttt{Settings.COMPUTE\_DTYPE}] Typ výpočtu pre 4-bit QLoRA.
	\item[\texttt{Settings.HOST}, \texttt{PORT}, \texttt{RELOAD}, \texttt{LOG\_LEVEL}] Nastavenia servera a logovania.
...
\begin{description}
	\item[\texttt{ModelService.load\_models()}] Načíta base model v 4-bit režime, pridá a zlúči LoRA adapter (ak existuje) a nastaví aktuálny model.
	\item[\texttt{ModelService.generate\_response()}] Generuje odpoveď z chat template, nastavuje stop tokeny podľa rodiny modelu a čistí špeciálne tokeny.
...
\texttt{generate\_response()} vypína \texttt{enable\_thinking} pre Qwen3, odstraňuje \texttt{<think>...</think>} bloky a pre UI konvertuje \texttt{###} a \texttt{**...**} na HTML tagy.
...
\begin{description}
	\item[\texttt{loadModels()}] Načíta dostupné modely (najprv \texttt{/models}, potom \texttt{/api/models}).
	...
	\item[\texttt{sendMessage()}] Odošle požiadavku na API, posiela \texttt{history} a \texttt{model} a aktualizuje lokálnu históriu.
\end{description}
...
\begin{description}
	...
	\item[\texttt{load\_and\_prepare\_data()}] Načíta JSONL s \texttt{prompt} a \texttt{completion}, tokenizuje a vytvorí train/test split.
\end{description}
...
\subsection{Spúšťacie skripty}

\textbf{\path{start_server.sh}} spustí API cez \texttt{energy\_chat/main.py} a preferuje \texttt{./venv/bin/python} ak existuje.

\textbf{\path{run_eval.sh}} aktivuje \texttt{venv}, doinštaluje \texttt{anthropic} (ak chýba) a spustí \texttt{scripts/evaluate_models.py}. Pre G-Eval je potrebné nastaviť \texttt{ANTHROPIC\_API\_KEY}.
...
\begin{description}
	...
	\item[\texttt{main()}] Pripraví dáta (\texttt{prompt/chosen/rejected}), spustí DPO tréning a uloží adapter; \texttt{--check-only} iba overí cesty.
\end{description}
